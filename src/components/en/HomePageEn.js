...
// 영문 홈 — HomePage.js 클론 (라벨 영문화 + /en 링크). 슬라이더/카테고리 텍스트는
// en/page.js가 번역 머지해 내려준다(미번역 시 KR 폴백).
export default function HomePageEn({ categories = [], featuredProducts = [], notices = [], companyInfo, sliders = [], partners = [] }) {
  ...
  // 슬라이더 link는 KR 경로로 저장돼 있으므로 /en 프리픽스 보정
  const enLink = (link) => {
    if (!link) return link;
    if (/^https?:\/\//.test(link)) return link;
    return link.startsWith('/en/') || link === '/en' ? link : `/en${link}`;
  };
  ...
  return (
    <>
      ...
      >
        {heroSlides.length > 0 ? (
                  ...
                  {slide.type === 'FULL_IMAGE' ? (
                    (() => {
                      const imgEl = slide.mobileImageUrl ? (
                        <picture>
                          ...
                          <img src={slide.imageUrl} alt={slide.title || 'Banner'} className="hero-fullimage" />
                        </picture>
                      ) : (
                        <img src={slide.imageUrl} alt={slide.title || 'Banner'} className="hero-fullimage" />
                      );
                      return slide.link ? (
                        <a href={enLink(slide.link)} className="hero-fullimage-link" aria-label={slide.title || 'Banner link'}>
                          {imgEl}
                        </a>
                      ) : (
                    ...
                    <div className="hero-content">
                      <div className="hero-text">
                        ...
                        <p className="hero-desc">{slide.description || 'Setting a new standard for industrial LED lighting'}</p>
                        <div className="hero-buttons">
                          {slide.link && (
                            <a href={enLink(slide.link)} className="hero-btn-primary">Learn More</a>
                          )}
                          <a href="/en/support/catalog" className="hero-btn-ghost">Catalog Request</a>
                        </div>
                      </div>
          ...
          <div className="hero-content">
            <div className="hero-text">
              ...
              <p className="hero-desc">Setting a new standard for industrial LED lighting</p>
              <div className="hero-buttons">
                <a href="/en/products" className="hero-btn-primary">View Products</a>
                <a href="/en/support/catalog" className="hero-btn-ghost">Catalog Request</a>
              </div>
            </div>
            <div className="hero-image">
        ...
        {hasMultipleSlides && (
          <>
            <button className="hero-arrow hero-arrow-prev" onClick={scrollPrev} aria-label="Previous slide">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <polyline points="15 18 9 12 15 6"/>
              </svg>
            </button>
            <button className="hero-arrow hero-arrow-next" onClick={scrollNext} aria-label="Next slide">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <div className="contact-strip-inner">
          <div className="contact-strip-item">
            ...
            <div className="contact-strip-text">
              <span className="contact-strip-label">Customer Center</span>
              <span className="contact-strip-value">+82-32-461-1800</span>
            </div>
          </div>
          <div className="contact-strip-item">
            <div className="contact-strip-icon">
            ...
            <div className="contact-strip-text">
              <span className="contact-strip-label">Business Hours</span>
              <span className="contact-strip-value">{companyInfo?.workingHoursEn || 'Mon–Fri 09:00–18:00 (KST)'}</span>
            </div>
          </div>
          <a
          ...
          >
            ...
            <div className="contact-strip-text">
              <span className="contact-strip-label">Video</span>
              <span className="contact-strip-value">Watch LVS Intro Video →</span>
            </div>
          </a>
        </div>
      </section>
      ...
      <section id="products-section" className={`products-lineup animate-on-scroll ${visibleSections.has('products-section') ? 'is-visible' : ''}`}>
        <div className="products-lineup-inner">
              ...
              <div className="category-showcase-grid">
                {productCategories[activeCategory]?.children?.map((subcat) => {
                  ...
                  const href = firstProductSlug
                    ? `/en/products/${firstProductSlug}`
                    : `/en/products/${productCategories[activeCategory].slug}?sub=${subcat.slug}`;
                  return (
          ...
          <div className="products-cta">
            <a href="/en/products" className="products-cta-btn">View All Products</a>
          </div>
        </div>
      </section>
        ...
        <div className="info-container">
          ...
          <div className="notice-section">
            <div className="notice-header">
              <h3>Notices</h3>
              <a href="/en/support/notices" className="notice-more">More +</a>
            </div>
            <div className="notice-list">
              {notices.slice(0, 4).map((notice) => (
                <a href={`/en/support/notices/${notice.id}`} key={notice.id} className="notice-list-item">
              ...
              {notices.length === 0 && (
                <p className="notice-empty">No notices have been posted yet.</p>
              )}
            </div>
          </div>
          ...
          <div className="services-grid">
            <a href="/en/support/consultation" className="service-card service-card-red">
            ...
            <a href="/en/support/contact" className="service-card service-card-blue">
              <div className="service-card-icon">
            ...
            <a href="/en/about/certifications" className="service-card service-card-green">
              <div className="service-card-icon">
