...
const UPLOAD_DIR = path.join(process.cwd(), 'public', 'images', 'dealers');
...
export async function POST(request) {
  try {
    ...
    // 최대 1200px 비율 유지 + WebP 변환 (로고 가독성 위해 q90부터 시작)
    ...
    const filename = `dealer-${Date.now()}-${random}.webp`;
    await writeFile(path.join(UPLOAD_DIR, filename), optimized.buffer);

    return NextResponse.json({
      filename,
      url: `/images/dealers/${filename}`,
